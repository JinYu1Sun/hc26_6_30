...
def chapter_overview(doc):
    ...
    for line in [
        ...
        "├── src/cloud_bridge_node.cpp           主节点：指令处理 + 状态上报",
        "├── launch/cloud_bridge.launch          启动参数",
        "├── CMakeLists.txt / package.xml        构建配置",
    ]:
        add_code(doc, line)
    add_para(doc, "整体数据流有两条：", bold=True)
...
def chapter_node_head(doc):
    add_heading(doc, "4. cloud_bridge_node.cpp —— 主节点", 1)
    add_para(doc, "主文件只包含一个类 CloudBridgeNode（MQTT 指令处理、ROS 话题收发、看门狗、"
                  "JSON 状态上报），外加最后的 main 函数。")
...
#include "cloud_bridge/mqtt_client.h\"""")
    ...
    add_heading(doc, "4.2 CloudBridgeNode 构造函数", 2)
    add_code(doc, """  CloudBridgeNode() : pnh_("~")
  {
    ...
    add_para(doc, "advertise<消息类型>(话题, 队列长度) 创建发布者；subscribe(话题, 队列, 成员函数指针, this) "
                  ...
                  "对定位、车辆状态这种“新数据覆盖旧数据”的场景能防止积压延迟。")
    ...
    add_para(doc, "createTimer 创建周期定时器：定位上报与车辆状态上报分别按 location_hz_/status_hz_ "
                  "换算周期，遥控看门狗固定 0.1s 一次。定时器回调同样由 spinner 线程执行。")

    add_heading(doc, "4.3 loadParams —— 读取 launch 参数", 2)
...
def chapter_node_cmd(doc):
    add_heading(doc, "4.4 onMqttMessage —— JSON 解析与指令分发", 2)
    add_code(doc, """  void onMqttMessage(const std::string& topic, const std::string& payload)
  {
    ...
    add_para(doc, "nlohmann::json::parse 把字符串解析成 JSON 对象，语法错误抛 json::exception。"
                  ...
                  "（比如 linear 给了字符串）。按 topic 字符串比较分发到三个处理函数，简单直接的命令路由。"
                  "对云端发来的畸形数据只警告不崩溃，保证节点稳健。")

    add_heading(doc, "4.5 handleMoveCmd —— 遥控移动", 2)
  ...
  void handleMoveCmd(const nlohmann::json& j)
  {
    ...
    add_heading(doc, "4.6 handleBladeCmd —— 刀盘控制", 2)
    ...
    add_heading(doc, "4.7 handleTaskCmd —— 自动任务 start/stop/pause/continue", 2)
    ...
    add_heading(doc, "4.8 startTaskSequence —— 任务启动序列", 2)
...
def chapter_node_rest(doc):
    add_heading(doc, "4.9 publishManualCmd —— uint16 补码回绕", 2)
    add_code(doc, """  void publishManualCmd(double linear, double angular)
  {
    ...
    add_para(doc, "把 [-1,1] 的比例值放大成下位机量程：drive_value = linear×drive_max_，"
                  "turn_value = -angular×turn_max_"
    ...
    add_heading(doc, "4.10 watchdogTimerCb —— 遥控看门狗", 2)
  ...
  void locationTimerCb(const ros::TimerEvent&)
  {
    ...
    add_heading(doc, "4.12 车辆状态回调与 statusTimerCb", 2)
  ...
  std::atomic<bool> task_seq_running_{false};""")
    add_para(doc, "成员分几组：ROS 句柄（NodeHandle/Publisher/Subscriber/Timer）；MQTT 客户端与主题名；"
                  ...
                  "车辆状态 status_mutex_）。选择标准是“要不要多个变量保持一致的快照”。")

    add_heading(doc, "4.14 main 函数", 2)
    add_code(doc, """int main(int argc, char** argv)
{
  ...
  return 0;
    ...
    add_para(doc, "ros::init 初始化节点；node 在栈上构造（构造函数里完成全部初始化）；"
                  ...
                  "ros::spin() 的并发版），某个回调耗时较长时不至于阻塞其它回调；"
                  "waitForShutdown 阻塞到 Ctrl+C/rosnode kill；退出前先 node.stop() 断开 MQTT，"
                  "然后栈展开自动析构 node（MqttClient 析构再兜底 stop）——"
                  "RAII 保证退出路径上资源都被清理。")
...
def chapter_launch(doc):
  ...
  <node pkg="cloud_bridge" type="cloud_bridge_node" name="cloud_bridge" output="screen">
    ...
    <param name="location_hz" value="10.0"/>
    <param name="status_hz" value="20.0"/>""")
    ...
    <param name="drive_max" value="15000"/>
    ...
    <param name="mow_height" value="11"/>""")
...
find_package(Threads REQUIRED)""")
    add_para(doc, "catkin 工作空间的标准开头。add_compile_options(-std=c++17) 启用 C++17"
                  ...
                  "Threads（std::thread 需要 -lpthread）单独找。")
...
find_library(PAHO_MQTT_C_LIB NAMES paho-mqtt3as paho-mqtt3a
  ...
  CATKIN_DEPENDS roscpp std_msgs mower_msgs util
)
...
target_link_libraries(cloud_bridge_node
  ...
  Threads::Threads
    ...
    add_para(doc, "catkin_package 声明本包对外的头文件目录和消息依赖（下游包 find_package 时用）。"
                  ...
                  "这是使用自定义消息时必须写的一行。"
                  "target_link_libraries 链接 catkin 库、两个 paho 库和线程库。"
  ...
  <description>MQTT cloud bridge for the mower: remote driving, blade control,
    auto task start/stop and location reporting.</description>
  ...
  <depend>util</depend>
    ...
    add_para(doc, "ROS 包清单：format=\"2\" 是较新的格式版本，<depend> 一条同时声明编译和运行依赖"
                  ...
                  "这里只列 ROS 包；nlohmann_json、paho 是系统库，不走 package.xml，"
...
def build_docx(path):
    ...
    add_para(doc, "ROS1 功能包 cloud_bridge 逐段代码讲解（C++ / ROS / MQTT）", size=Pt(10.5))

    chapter_overview(doc)
    chapter_header(doc)
    chapter_client_cpp(doc)
    chapter_node_head(doc)
    chapter_node_cmd(doc)
