...
private:
    ...
    void vehicleCmdCallback(const mower_msgs::VehicleCmd::ConstPtr &msg) {
        // 生成UDP数据包
        std::vector<uint8_t> udp_data = createUdpPacket(
            msg->mover_bool,
            msg->drive_value,
            msg->turn_value,
            msg->mower_height);
        ...
        // if ((now - last_log_time).toSec() > 0.1) {
            ROS_INFO("Sending command: drive=%d, turn=%d,mover_bool=%d, mower_height=%d", 
                    msg->drive_value, msg->turn_value,msg->mover_bool, msg->mower_height);
        //     last_log_time = now;
        // }
    }
    ...
    void sendTestFrame() {
        ...
        // 测试数据：drive=50, turn=20, mower_height=1
        std::vector<uint8_t> test_data = createUdpPacket(0, 50, 20, 1);
    ...
    void sendStopCommand() {
        ...
        // 停止命令：drive=0, turn=0, mower_height=0
