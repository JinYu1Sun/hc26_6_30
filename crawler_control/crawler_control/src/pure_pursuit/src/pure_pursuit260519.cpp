...
void PurePursuit::setControl_main(void) {
      ...
      if (turn_flag) {
        ...
        if (distance_to_start > 0.05 && !adjust_error) {
          if (fabsf(heading_error) > 0.05 && !first_set) {
            ...
            command.turn_value = (heading_error > 0) ? -30 : 30; // 1270 : 1730
        ...
        ROS_WARN("delta_heading,%.4f",
                  delta_heading);
        if (delta_heading < 0) {
          command.turn_value = -30; // 1270
          command.drive_value = 0; // 1500
          break;
        } else {
          command.turn_value =30; // 1730
          command.drive_value = 0; // 1500
          break;
        }
      }
  ...
  if (command.turn_value >= -30 && command.turn_value <= 30) { // 2300
...
void PurePursuit::callbackFromPlanningResult(
  ...
  if (need_turn_for_new_path) {
    ...
    if (fabsf(heading) <= 0.10) {
      ...
      command.turn_value = (heading > 0) ? -30 : 30; // 1270 : 1730
