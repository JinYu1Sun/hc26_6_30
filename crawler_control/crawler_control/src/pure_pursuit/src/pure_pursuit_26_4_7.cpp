...
void PurePursuit::setControl_main(void) {
      ...
      if (turn_flag) {
        ...
        if (distance_to_start > 0.05 && !adjust_error) {
          if (fabsf(heading_error) > 0.05 && !first_set) {
            ...
            command.turn_value = (heading_error > 0) ? -45 : 45; // 1270 : 1730
            count = 0;
        ...
        if (fabsf(local_waypoints[0].heading) <= 0.10) // 0.096
        {
        ...
        ROS_WARN("delta_heading,%.4f",
                  delta_heading);
        if (delta_heading < 0) {
          command.turn_value = -45; // 1270
          command.drive_value = 0; // 1500
          break;
        } else {
          command.turn_value = 45; // 1730
          command.drive_value = 0; // 1500
          break;
        }
      }
  ...
  if (command.turn_value >= 45 && command.turn_value <= 45) { // 2300
    command.turn_value = command.turn_value;
  } else {
    command.turn_value = ctv;
  }
  if (command.drive_value >= -45 && command.drive_value <= 45) { // 2300
    command.drive_value = command.drive_value;
  } else {
    command.drive_value = 20; // 1900
  }
...
robot::geometry::NormalizeAngle(planning_result.heading[0] - car_position.yaw);
        ...
        if (fabsf(heading) <= 0.096)
        {
            ...
            initial_approach_mode = true; // cnk 0514 设置初始接近模式
            approach_count = 0;
            ResetFlag(); //
            // command.drive_value = 1730; // 1750
        }
  ...
  if (need_turn_for_new_path) {
    if (align_to_path_point && planning_result.pathtype == 0) {
      ...
      if (distance_to_start > 0.05) {
        if (fabsf(heading_error) > 0.05 && !already_moving_straight) {
          command.drive_value = 0; // 1500
    ...
    if (fabsf(heading) <= 0.10) {
      ...
      command.turn_value = (heading > 0) ? -45 : 45; // 1270 : 1730
...
int calculateTurnValue(bool latest_gear_flag, waypose local_waypose,
    ...
    // ==================== 参数配置区 ====================
    const int ctv = 0;           // 中位值（直行值），根据实际修改
    const int MIN_TV = -20;        // 最小转向值（左满舵）
    const int MAX_TV = 20;        // 最大转向值（右满舵）
    // ===================================================
    
    static bool dev_flag = 0;
    static bool sl_flag = 0;
    ...
    int x_sign = local_waypose.point.x > 0 ? 1 : -1;
    int h_sign = local_waypose.heading > 0 ? 1 : -1;
    
    float h_value = latest_gear_flag == 0 ? local_waypose.heading : 0;
    ...
    if (fabs(local_waypose.point.x) < 0.10 &&
        ...
        if (h_sign == 1) {
            det_tv = (x_sign == 1) ? 5 : -10;
        } else {
            det_tv = (x_sign == 1) ? 10 : -5;
        }
        
        t_value = last_tv + det_tv;
        
        // ===== 限幅保护 =====
        if (t_value > MAX_TV) t_value = MAX_TV;
        if (t_value < MIN_TV) t_value = MIN_TV;
        // ===================
