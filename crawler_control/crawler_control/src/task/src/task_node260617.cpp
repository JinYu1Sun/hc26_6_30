...
void controlFigure8()
{
    if (!init_mode || !init_confirmed)
        return;
    mower_msgs::VehicleCmd cmd_msg;
    cmd_msg.drive_value = 30;
    cmd_msg.turn_value = -47; // 向左转，持续走圆

    // 其他参数
    cmd_msg.ad_control_enable = 1;           // 自动驾驶控制开启
    cmd_msg.gear_model = 3;                  // 前进档
    cmd_msg.mover_bool = 0;                  // 不割草
    cmd_msg.mower_height = 0;                // 割草高度（随便设一个）
...
void SingalCallBack(const std_msgs::String &singal_msgs)
{
        ...
        if (pid_controller_pid == -1)
        {
            ...
            if (pid_controller_pid == 0)
            {
                ...
                execl("/bin/bash", "bash", "-c",
                      // "source /home/oem/tanke/crawler_control/devel/setup.bash && rosrun pure_pursuit pure_pursuit",
                      "source /home/nvidia/crawler_control/devel/setup.bash && rosrun pure_pursuit pure_pursuit",
                      (char *)0);
                // 如果 execl 失败
                perror("execl failed");
                exit(1);
    ...
    else if (signal == "stop_execution")
    {
        if (pid_controller_pid > 0)
        {
            //system("rosnode kill /pure_pursuit");
            ROS_INFO("Stopping pp_controller with PGID: %d", pid_controller_pid);

            // 杀掉整个进程组（负号表示 PGID）
            if (kill(-pid_controller_pid, SIGKILL) == 0)
            {
                ROS_INFO("SIGTERM sent to process group %d", pid_controller_pid);
            }
            else
            {
                ROS_ERROR("Failed to send SIGTERM to process group %d", pid_controller_pid);
            }

            // 等待子进程退出（清理僵尸）
            waitpid(pid_controller_pid, nullptr, WNOHANG);
            pid_controller_pid = -1;
        }
        else
        {
    ...
    cout << "APP(appsignal_flag) @ " << (int)appsignal_flag << endl; // app
}

//@xhj add FusionMAP state to fault_code[2]
void FusionMapCallBack(const mower_msgs::Position &msgs)
{
    check_fusion = (msgs.position_state == 1 || msgs.position_state == 2) ? 1 : 0;
    fault.fault_code[2] = check_fusion ? "0" : "1";
    monitor.node_normal[2] = check_fusion ? 1 : 0;

    // 检测定位状态
    current_has_position = check_fusion; //(msgs.position_state == -1);//??????

    // 如果从无定位变为有定位，且正在初始化模式，则退出初始化
    if (has_position && current_has_position && init_mode)
    {
        init_mode = false;
        init_requested = false;
        init_confirmed = false;
    ...
    // has_position = current_has_position;
    has_position = check_fusion;
...
{
    if (!manual_better) // 停车置true, 其它情况进循环
    {
        if (!warn_2)
        {
            ...
            {
                taskstatus = Passing_connecting_space;
            }
        }
    }
}
...
{
    // cout<<"front(Yolo_flag)&(V_dir) @ "<<(int)range_msgs.data<<" &&  "<<rangewarn_flag<<endl; //履带车停障判断
...
/*
void RangefrontCallBack(const std_msgs::Bool &range_msgs)
{
  if (range_msgs.data && rangewarn_flag != 1)
  {
    front_flag = true;
    cout<<"front  stop~~front stop~~front stop~~"<<endl;
  }
  else
  {
    front_flag = false;
...
void RangerearCallBack(const std_msgs::Bool &range_msgs)
{
  if (range_msgs.data && rangewarn_flag != 3)
  {
    rear_flag = true;
    cout<<"rear stop@@rear stop rear stop@@"<<endl;
  }
  else
  {
    rear_flag = false;
...
bool checkAndResetNodes(const mower_msgs::Monitor &msgs)
{
    bool all_true = true;
    monitor = msgs;
    monitor.node_normal[2] = 1; // 测试
    pub_monitor.publish(monitor);
    // for (int i = 0; i < 13; ++i){
    //   if (!monitor.node_normal[i]){
    //     all_true = false;
    //     //std::cout << "$ Node " << i << " (" << node_names[i] << ") is not normal. " << std::endl;
    //   }
    // }

    bool test = monitor.node_normal[2];
    // 将所有标志位 置为false
    for (int i = 0; i < 13; ++i)
    {
...
int main(int argc, char **argv)
{
    ...
    // sub_manual = nh.subscribe("/mower/manual_set", 1, ManualCallBack);
    ...
    // sub
    sub_plantype = nh.subscribe("/mower/plan_type_state", 1, PlanTypeCallBack);
    sub_controlstate = nh.subscribe("/mower/control_state", 1, ControlStateCallBack);

    // sub_warn = nh.subscribe("/mower/warning", 1, WarnCallBack);  //@ 0819 未存续
    // sub safe
    ...
    // ros::Subscriber sub_rangefront = nh.subscribe("/Mower/front_warning", 1, RangefrontCallBack);  //@xhj 2406
    // ros::Subscriber sub_rangerear = nh.subscribe("/Mower/rear_warning", 1, RangerearCallBack);    //@xhj 2406
    ...
    // pub_fault = nh.advertise<mower_msgs::Fault_Code>("/mower/fault_code", 1);
        ...
        if (init_mode)
        {
            ...
            if (!check_fusion)
            {
                ...
                else
                {
                    ...
                    // check_fusion = true; // 强行跳过后续逻辑（也可以用 done 标志位）
                }
