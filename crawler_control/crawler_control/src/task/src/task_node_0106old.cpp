...
// "8"字形控制函数
void controlFigure8() {
    if (!init_mode || !init_confirmed) return;
    mower_msgs::VehicleCmd cmd_msg;
    cmd_msg.drive_value = 1810;
    cmd_msg.turn_value = 1650;  // 向左转，持续走圆
...
{
  check_fusion = msgs.position_state == 2 ? 1 : 0;
  fault.fault_code[2] = check_fusion ?"0":"1";
  monitor.node_normal[2] = check_fusion ? 1 : 0 ;
  
  // 检测定位状态
  current_has_position = (msgs.position_state == -1);
    ...
    if (init_mode)
    {
        ...
        if (!straight_done)
        {
            if (!straight_active)
            {
                ...
                ros::Duration(13.0).sleep();//等待RTK重启
