...
void PurePursuit::run() {
  ...
  while (ros::ok()) {
    ...
    if(!is_carposition_set_ || stop_car_flag)
    {
        ...
        stop_cmd.turn_value = 1500;
        stop_cmd.drive_value = 1500; // 停车
    ...
    if (was_reversing && avoid_state_data != 4) {
      ...
      // 先发送停车命令，确保电机停止
      if (transition_time < 5) { // 增加到0.3秒，确保完全停止
        mower_msgs::VehicleCmd stop_cmd;
        stop_cmd.turn_value = 1500;
        stop_cmd.drive_value = 1500; // 停车
        stop_cmd.ad_control_enable = 1;
    ...
    // //驾驶模式临时修改
    if (is_planning_result_set_ && is_carposition_set_ && !was_reversing) // 驾驶模式临时修改，添加过渡状态检查
