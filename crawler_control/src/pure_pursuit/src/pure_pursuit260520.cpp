...
int calculateTurnValue(bool latest_gear_flag, waypose local_waypose,
  ...
  static float h = 0;
  int det_tv = 0;
  // if (abs(last_tv - 0) < 2) { // 0是1500，0.025对应10
  //   sl_flag = 1;
  //   cu_flag = 0;
  // } else {
  //   sl_flag = 0;
  ...
  // fabs(local_waypose.point.x) < 0.10 &&
  //     fabs(-local_waypose.heading) > 0.05 &&
  // if ( cu_flag) {
    int tmp_value = 0;
    // h_sign ？向左打：向右打
    if (h_sign == 1) {
  ...
  //       det_tv = abs(det_tv) < 2.5 ? det_tv : sign * 2.5; // 最大调整步幅不超过
  //     }
