...
void PurePursuit::setControl_main(void) {
  int gear_count = 0;
  desire_speed_com = planning_result.speed[nearest_waypoint_id] * 100.0f;//厘米级单位
  int ld_init;
 
  ld_init = desire_speed_com < 25 ? ld_i + 2 : (desire_speed_com < 60 ? ld_i + 5 : ld_i + 8);   //}
      ...
      if (turn_flag) {
        ...
        if (distance_to_start > 0.05 && !adjust_error) {
          if (fabsf(heading_error) > 0.05 && !first_set) {
            ...
            command.turn_value = (heading_error > 0) ? -12 : 12; // 1270 : 1730
        ...
        ROS_WARN("delta_heading,%.4f",
                  delta_heading);
        if (delta_heading < 0) {
          command.turn_value = -12; // 1270
          command.drive_value = 0; // 1500
          break;
        } else {
          command.turn_value =12; // 1730
          command.drive_value = 0; // 1500
          break;
        }
      }
  ...
  if (command.turn_value >= -12 && command.turn_value <= 12) { // 2300
    command.turn_value = command.turn_value;
  } else {
    command.turn_value = ctv;
  }
  if (command.drive_value >= -100 && command.drive_value <= 100) { // 2300
    command.drive_value = command.drive_value;
  } else {
    command.drive_value = 100; // 1900
  }
...
void PurePursuit::callbackFromPlanningResult(
  ...
  if (need_turn_for_new_path) {
    ...
    if (fabsf(heading) <= 0.10) {
      ...
      command.turn_value = (heading > 0) ? -12 : 12; // 1270 : 1730
...
int calculateTurnValue(bool latest_gear_flag, waypose local_waypose,
  ...
  if (fabs(c_goal->heading - h) > 0.02 ||
    ...
    turn_max = 1.5*(desire_speed_com/10)>6?1.5*(desire_speed_com/10):6;
    if (t_value > turn_max) {
    t_value = turn_max;
    } else if (t_value < -turn_max) {
        t_value = -turn_max;
    }
  } else { // 250312 add
        t_value = 0;
