...
void PurePursuit::setControl_main(void) {
  ...
  int target_idx = control_base_idx + static_cast<int>(std::round(lookahead_dist_m / kPointSpacing));
 ...
 ROS_INFO("[setControl_main] lookahead: v=%.3f m/s, gear2_idx=%d, target_idx=%d,turn_target_idx_=%d",
           v_m_s, gear2_idx, target_idx, turn_target_idx_);
  ...
  if (turn_phase_ == TurnPhase::APPROACH) {
    ...
    if (dist_to_turn < kDecelStartDist) {
      ...
      if (approach_speed < 0.10f) {
        approach_speed = 0.10f;  // 保留最小速度，避免完全停滞
      }
    }
...
int calculateDriveValue(float current_speed, float desire_speed, bool flag,
  ...
  ROSINFO("calculateDriveValue: avoid_state_data=%.3f ",avoid_state_data);                      
  if (flag) {
    return 0; // 停车/转弯状态
  }
  if (avoid_state_data == 3) {
