...
void PurePursuit::setControl_main(void) {
  ...
  // 安全兜底：如果转弯状态机中保存的目标点越界，强制退出 APPROACH / EXECUTING。
  // 注意：不因为 gear[turn_target_idx_] != 2 就 reset，因为路径更新有延迟，
  // 短暂不一致时应保持当前转弯状态，避免在 delay 中反复进入/退出。
  if (turn_phase_ != TurnPhase::NONE) {
    if (turn_target_idx_ < 0 || turn_target_idx_ >= planning_result.point_num) {
      ROS_WARN("[setControl_main] turn target %d out of bounds, reset turn phase to NONE",
               turn_target_idx_);
  ...
  // 用最近路径点的航向，而不是固定取 local_waypoints[0]
  ctlE.e_theta = local_waypoints[nearest_waypoint_id].heading * 180.0f / M_PI;

  // ========== 3. 基于距离选择目标预瞄索引 ==========
  // 已知相邻路径点间距约为 0.25m
  // 预瞄距离根据实际车速动态选择（对路径延迟更鲁棒），纵向控制仍用规划车速。
  ...
  const float kMinLookaheadSpeedMps = 0.1f;  // 最小车速，防止实际速度为 0 时预瞄过近
  float actual_speed_m_s = fabsf(vehicle_info.vehicle_speed) / 100.0f;
  float lookahead_speed_m_s = std::max(actual_speed_m_s, kMinLookaheadSpeedMps);
  float lookahead_dist_m = weight_l * lookahead_speed_m_s + 0.3f;
  ...
  // 入口保护：不要重新进入刚刚完成的同一个转弯点
  if (gear2_idx != -1 && has_last_completed_turn_ &&
      ros::Time::now() < turn_completion_guard_end_time_) {
    ...
    if (std::hypot(dx_comp, dy_comp) < 0.1f) {
      ROS_INFO("[setControl_main] ignore just-completed turn point at (%.2f,%.2f)",
               last_completed_turn_x_, last_completed_turn_y_);
      gear2_idx = -1;
    }
  }

  if (gear2_idx != -1 && turn_phase_ == TurnPhase::NONE) {
    turn_phase_ = TurnPhase::APPROACH;
    turn_target_idx_ = gear2_idx;
    ROS_INFO("[setControl_main] APPROACH entered: idx=%d", gear2_idx);
  }

  // 一旦处于转弯状态机，强制预瞄到 latch 的转弯点
  if ((turn_phase_ == TurnPhase::APPROACH || turn_phase_ == TurnPhase::EXECUTING) &&
      turn_target_idx_ >= 0 && turn_target_idx_ < planning_result.point_num) {
    target_idx = turn_target_idx_;
    look_ahead_index = target_idx;
  }

  ROS_INFO("[setControl_main] lookahead: v_plan=%.3f m/s, v_actual=%.3f m/s, "
           "gear2_idx=%d, target_idx=%d, turn_phase=%d",
           nearest_speed_m_s, lookahead_speed_m_s, gear2_idx, target_idx,
           static_cast<int>(turn_phase_));
  ...
  if (turn_phase_ == TurnPhase::APPROACH) {
    ...
    // --- 新增：如果 latch 的转弯点已经不在前方路径里，直接退出 APPROACH ---
    if (!(turn_target_idx_ >= 0 && turn_target_idx_ < planning_result.point_num &&
         planning_result.gear[turn_target_idx_] == 2 &&
         turn_target_idx_ >= nearest_waypoint_id)) {
      ROS_WARN("[setControl_main] turn target invalid/disappeared, abort APPROACH");
      turn_phase_ = TurnPhase::NONE;
      turn_target_idx_ = -1;
    }

    // 空间条件满足，才进入 EXECUTING
  ...
  if (turn_phase_ == TurnPhase::EXECUTING) {
    ...
    int next_idx = turn_target_idx_ + 1;
    if (next_idx < planning_result.point_num) {
      float dx = local_waypoints[next_idx].point.x - local_waypoints[turn_target_idx_].point.x;
      float dy = local_waypoints[next_idx].point.y - local_waypoints[turn_target_idx_].point.y;
      heading_error = atan2f(dy, dx);
    } else if (turn_target_idx_ < planning_result.point_num) {
      heading_error = local_waypoints[turn_target_idx_].heading;
    }
    ...
    if (std::fabs(heading_error) <= 0.10f) {
      ...
      // 记录刚完成的转弯点，避免 planner 还没删掉旧 gear=2 时再次进入同一转弯点
      if (look_ahead_index >= 0 && look_ahead_index < planning_result.point_num) {
        last_completed_turn_x_ = planning_result.x[look_ahead_index];
        last_completed_turn_y_ = planning_result.y[look_ahead_index];
        has_last_completed_turn_ = true;
        turn_completion_guard_end_time_ = ros::Time::now() + ros::Duration(1.0);
      }
  ...
  // 如果已经退出转弯状态机，重新按正常预瞄距离计算 target_idx，
  // 避免继续使用 latch 的转弯目标或 gear==2 点
  if (turn_phase_ == TurnPhase::NONE) {
    target_idx = control_base_idx +
                 static_cast<int>(std::round(lookahead_dist_m / kPointSpacing));
    if (target_idx >= planning_result.point_num) {
      target_idx = planning_result.point_num - 1;
    }
  } else if (turn_target_idx_ >= 0 && turn_target_idx_ < planning_result.point_num) {
    // 仍在转弯状态机中，确保预瞄点跟随最新的转弯目标
    target_idx = turn_target_idx_;
  }
  look_ahead_index = target_idx;
  // target_idx 已在上文 clamp 到 [0, point_num-1]
  ...
  if (avoid_state_data != 1) {
    if (!c_finish && path_state == PATH_FINISHED &&
        // ROS 坐标系下 x 为前进方向
        fabsf(local_waypoints[look_ahead_index].point.x) < 0.2f && fabsf(vehicle_info.vehicle_speed) > 2) {
      std::string filePath =
      ...
      float c_length =
          sqrt(pow(local_waypoints[look_ahead_index].point.x, 2.0f) + pow(local_waypoints[look_ahead_index].point.y, 2.0f)) * 100; // cm
      if (c_length < 10) {
