...
ControlError ctlE = {44.4f, 44.4f, 44.4f}; //@xhj add Error feedback, data_init

int ld_i = 4;        // 2
bool turn_flag = 0;  // 正式转弯标志位
bool gear_flag = 0;  // 档位标记的转弯点，第一次等于1，意味离转弯点仍有1个规划距离（50cm）
...
void ResetFlag()
{
    ld_i = 4;        // 2
    turn_flag = 0;  // 正式转弯标志位
    gear_flag = 0;  // 档位标记的转弯点，第一次等于1，意味离转弯点仍有1个规划距离（50cm）
...
void PurePursuit::setPath_main(void)
{
    ...
    for (i = 0; i < planning_result.point_num; i++)
    {
        ...
        p1.heading = robot::geometry::NormalizeAngle(planning_result.heading[i] - car_position.yaw); // 局部点位航向角
        ...
        // if(i<=9){
        // // ROS_INFO("D1#localpath# x:%.3f,y:%.3f,heading:%.3f", local_waypoints[i].point.x,local_waypoints[i].point.y,local_waypoints[i].heading); //debug_1
        // }
    }
}
...
{
    float distance;
    if (planning_result.point_num == 0 || planning_result.point_num < look_ahead_index)
...
int PurePursuit::updateLatTrackState(void)
{
    ...
    case CONTROL_OUT_OF_BOUND:
        ...
        {
            ROS_INFO("CONTROL_OUT_OF_BOUND ~dis~  %.3f", lat_error); //@xhj
...
int PurePursuit::getNearestIndex(int *index)
{
    ...
    *index = -1;
    ...
    for (i = 0; i < planning_result.point_num; i++)
    {
        ...
        /*if (local_waypoints[i].point.y <= 0.0f) // 局部转换，前向y正值最近的点 @add
            continue;
        nearest_index = i;
        break;*/
    }
    ...
    if (!nearest_index)
    {
        ...
        // ROS_INFO("D33#nearest _index:%d,near_i:%d,x:%.3f,y:%.3f", nearest_index,near_i,
        //    local_waypoints[near_i].point.x,local_waypoints[near_i].point.y);
    }
    // ROS_INFO("D3#nearest _index:%d,dis:%.3f,x:%.3f,y:%.3f", nearest_index,nearest_dis,
    //    local_waypoints[nearest_index].point.x,local_waypoints[nearest_index].point.y); //debug_3
    ...
    {
        // ROS_INFO("PATH_TOO_FAR ~dis~  %.3f", nearest_dis); //@xhj
        return PATH_TOO_FAR;
    }
    *index = nearest_index;
    return 1;
...
int PurePursuit::getLatDistance(float *distance)
{ //@xhj:1224 debug == 0和航向角有关
    *distance = 999.99f;
    if (nearest_waypoint_id > planning_result.point_num)
        return 0;

    if (nearest_waypoint_id == 0 /*&& local_waypoints[nearest_waypoint_id].point.y < 0.20*/)
    {
...
// 计算车辆当前位置（假设坐标为 (0,0)，即原点）到由两点 ∗p1 和 ∗p2 定义的直线的垂直距离
float PurePursuit::getDistanceToLine(waypose *p1, waypose *p2)
{ // 与车坐标点（0，0）算距离
...
{
    if (planning_result.point_num < 1) //@xhj :2504 <=改为 <
        return 0;
    ...
    // ROS_INFO("D0goal->y:%.3f,x:%.3f,heading:%.3f,ld_index:%d", goal->point.y, goal->point.x,goal->heading,goal_id); //debug_0
    return 1;
}
...
int PurePursuit::isGoalValid(void)
{
    ...
    if (nearest_waypoint_id < 0 || look_ahead_index < 0 || planning_result.point_num < look_ahead_index)
        return 0;
    ...
    for (i = nearest_waypoint_id; i < look_ahead_index; i++)
    {
        if (isinf(desire_ICR))
            ...
            lat_error_temp = fabsf(
                sqrtf(powf(local_waypoints[i].point.x * 100 - desire_ICR, 2.0f) + powf(local_waypoints[i].point.y * 100, 2.0f)) - fabsf(desire_ICR)); // 局部路径圆与行走半径圆差异值

        if (lat_error_temp > max_goal_lat_error)
            max_goal_lat_error = lat_error_temp;

        // ROS_INFO("D6#lat_error_temp %.3f,ICR %.3f", lat_error_temp,desire_ICR);  //debug_6
...
void PurePursuit::setControl_main(void)
{
    ...
    {
        /////////////////////////// cnk 0514 初始接近模式 ///////////////////////////
        //   if (initial_approach_mode && planning_result.gear[SPEED_TARGET_POINT] != 2 )
        //  {
            //    approach_count++;
        //     float lateral_error = fabsf(local_waypoints[0].point.x);
        //     float heading_error = fabsf(local_waypoints[0].heading);
        //     if (lateral_error > 0.1)// || heading_error > 0.2) // 横向误差过大
        //     {
        //         float turn_correction = 0;
        //         if (lateral_error > 0.1) 
        //         {
        //             int direction = local_waypoints[0].point.x > 0 ? 1 : -1;
        //             turn_correction += direction * 180;
        //         }
        //         if (heading_error > 0.2) 
        //         {
        //             int hdg_direction = local_waypoints[0].heading > 0 ? 1 : -1;
        //             turn_correction += hdg_direction * 100;
        //          }
        //         turn_correction = std::max(-250.0f, std::min(250.0f, turn_correction));
        //         command.turn_value = 1500 + static_cast<int>(turn_correction);
        //         command.drive_value = 1650; // 降低速度
        //         ROS_INFO("Initial approach mode: lateral error %.3f m", lateral_error);
        //         return;
        //     }
            //   if (approach_count > 40) // 一段时间后退出靠近模式
        //     {
                //  initial_approach_mode = false;
        //         ROS_INFO("Exiting initial approach mode");
            //  }
        // 
        /////////////////////////// cnk 0514 初始接近模式 ///////////////////////////
        ...
        for (int i = 0; i < planning_result.point_num; i++)
        {
            ...
            {
                gear_flag = 0;
                turn_state = planning_result.gear[SPEED_TARGET_POINT + 1] == 2 ? 0 : turn_state; // 提前预警转弯
            }
            else
            {
                gear_flag = planning_result.gear[SPEED_TARGET_POINT] == 2 ? 1 : 0; //@
            }
            // turn_state = planning_result.point_num == 1 ? 0 : turn_state; //@ 最后一个点0，新路径马上接2
            //@xhj 0109 add stop_tem_flag 旋转要求
            if (turn_flag || (gear_flag && local_waypoints[0].point.y < 0.09))
            {                                        // TODO 0.09是否合适 cnk
                ...
                // ROS_INFO("Ds4#det %.3f,t2 %.3f,t1 %.3f", det,t2.toSec(),t1.toSec());  //debug_s
                // ROS_INFO("Dsdelat_Vy =  %.3f", add_y*100 / det);  //连续几帧速度判定 cm/s
                if (det >= 0.2)
                {
                ...
                {
                    stop_tem_flag = 1; // det<0.2 停车      // @chenli
        ...
        ////////////// cnk 0514
        look_ahead_index = look_ahead_index <= ld_init ? look_ahead_index : ld_init;                                                                     //@xhj 0107 限制前视最远点
        // ROS_INFO("D44#ld_index %d,gear_flag %d,stop_tem %d,turn_flag %d,t_state %d", look_ahead_index, gear_flag, stop_tem_flag, turn_flag, turn_state); // debug_44
    }
    ROS_INFO("D44#desire_speed_com %f,gear_flag %d,stop_tem %d,turn_flag %d,t_state %d", desire_speed_com, gear_flag, stop_tem_flag, turn_flag, turn_state);
    vir_wheel_track = 1.0 * wheel_track; // 标定  与滑动摩擦有关
    // pl0_heading = planning_result.heading[0];
    ctlE.e_theta = (local_waypoints[0].heading - 0) * 180 / M_PI; // 与期望航向0的差值 单位：度
    // ROS_INFO("D4#point_num %d,V_des %.3f cm/s,ld_init: %d", planning_result.point_num,desire_speed_com,ld_init);  //debug_4
    ...
    while (!stop_tem_flag) // 1
    {
        ...
        // command.drive_value = calculateDriveValue(desire_speed_com,planning_result.speed[look_ahead_index-1],turn_flag, avoid_state_data);
        // look_ahead_index-1 会报段错误
        command.drive_value = calculateDriveValue(desire_speed_com, planning_result.speed[look_ahead_index], turn_flag, avoid_state_data);
        ...
        if (desire_speed_com <= 25.0 && planning_result.point_num > 1)
        {
            ...
            // ROS_INFO("D05#near_wp_id %d,delta_heading %.4f,l[0] %.3f,near %.3f", nearest_waypoint_id,delta_heading,
            //          local_waypoints[0].heading,local_waypoints[nearest_waypoint_id].heading);  //debug_05
            // 自旋转向
            ...
            if (turn_flag)
            {
                ...
                ROS_WARN("start turn, %.4f,%.4f",planning_result.x[0],planning_result.y[0]);                // 0.1弧度=5.73度
                if (fabsf(local_waypoints[0].heading) <= 0.06) //0.096
                {
                    ...
                    ROS_WARN("gear =2 should droped,turn finish, %.4f,%.4f",planning_result.x[0],planning_result.y[0]);
                    turn_state = 1;
                    turn_flag = 0;
        ...
        if (!turn_flag)
        {
            ...
            {
                if (fabsf(goal->heading) <= 0.02 && fabsf(ctlE.e_pos) < 0.06) // 假定预瞄点在正前方
                {
                    // 需要速度反馈，对值PID
                    command.turn_value = ctv;
                    desire_ICR = INFINITY;
                ...
                {
                    desire_ICR = (pow(goal->point.x * 100, 2.0f) + pow(goal->point.y * 100, 2.0f)) / (2.0f * goal->point.x * 100); // 带方向 正 右转，负 左转
                    command.turn_value = calculateTurnValue(gear_flag, local_waypoints[0], goal, goal_id, last_turn_value);
                    break;
                }
            }
            else // 没有找到预瞄点,停车
            ...
            if (isGoalValid()) // 当前转向能够保证预瞄点以内路径横向偏差
            {
                ...
                if (findGoalPoint()) // 更新 预瞄点和goal, <= ld_i
                {
                    ...
                    {
                        float desire_ICR = (pow(goal->point.x * 100, 2.0f) + pow(goal->point.y * 100, 2.0f)) / (2.0f * goal->point.x * 100);
                        command.turn_value = calculateTurnValue(gear_flag, local_waypoints[0], goal, goal_id, last_turn_value);
                        break;
                    }
                }
            ...
            else if (fabs(goal->point.y < 0.9f) && (goal_id < 2)) // 当前转向不能够保证预瞄点以内路径横向偏差，预瞄点过近，就这样开吧
            {
                // @xhj 1230 判断加fabs
                // ROS_INFO("Continue driving %d,%f,%d,%d", (goal->point.y < 3.0f), goal->point.y, (goal_id < 2), goal_id);
                break;
            }
            else
            { // 当前转向不能够保证预瞄点以内路径横向偏差，预瞄距离可以减小
                look_ahead_index--;
    ...
    // ROS_INFO("D9#cmd# t_v:%d,d_v:%d,~~tv %d,dv %d", command.turn_value, command.drive_value, tv, dv);

    //@xhj ： add c_finished
    if (avoid_state_data != 1)
    {
        if (!c_finish && path_state == PATH_FINISHED && fabsf(goal->point.y) < 0.2f && fabsf(vehicle_info.vehicle_speed) > 2)
        {
            // ROS_INFO("path_state is PATH_FINISHED and goal->point.y = %.3f", goal->point.y);
            std::string filePath = "/home/nvidia/Mower/src/location_map/ControlStatus.txt";
            ...
            float c_length = sqrt(pow(goal->point.x, 2.0f) + pow(goal->point.y, 2.0f)) * 100; // cm
            if (c_length < 10)
            {
        ...
        {
            // ROS_INFO("path_state: %d,finish: %d,goal->point.y: %.3f,track_state: %d", path_state, c_finish, goal->point.y,lat_track_state); //@xhj-debug: add
            c_finish = false;
        }
    }
}
...
void PurePursuit::act(void)
{
    ...
    case PATH_INVALID:
        ...
        break;
    case PATH_TOO_FAR:
        stop_path_flag = 1; //@xhj add
        // ROS_INFO("PATH_TOO_FAR ~stop_ =  %d", stop_path_flag); //@xhj
        break;
    case PATH_INIT:
    case PATH_FINISHED:
    default:
...
void PurePursuit::callbackFromPlanningResult(const util::LocalPathConstPtr &msg_planning_result)
{
    ...
    error_status = (planning_result.point_num > 50 || planning_result.point_num < 1) ? 1 : 0;
    if (error_status)
    {
        is_planning_result_set_ = false;
    ...
    if (need_turn_for_new_path)
    {
        ...
        double heading = robot::geometry::NormalizeAngle(planning_result.heading[0] - car_position.yaw);
...
{
    planning_result = *msg_planning_result;
    error_status = (planning_result.point_num < 1) ? 1 : 0; //planning_result.point_num > 50 || 
    if (error_status)
    {
        is_planning_result_set_ = false;
        ROS_ERROR_THROTTLE(1, "~ point_num is wrong : %d ", planning_result.point_num);
    ...
    if (need_turn_for_new_path)  
    {
        if (align_to_path_point && planning_result.pathtype == 0)
        {
            ...
            double heading_error = robot::geometry::NormalizeAngle(desired_heading - car_position.yaw);
            ROS_INFO("Align to path point: distance = %.2f, heading_error = %.3f", distance_to_start, heading_error);
...
void PurePursuit::command_publish()
{
    ...
    //@xhj add flag to car_stop_status
    // if (stop_car_flag || c_finish ||stop_tem_flag || stop_path_flag)
    if (stop_car_flag || c_finish || (stop_tem_flag && !need_turn_for_new_path) || stop_path_flag) // cnk 0512
    {
        ...
        last_turn_value = (last_turn_value < 1600 && last_turn_value > 1400) ? last_turn_value : 1500;
        ...
        // ROS_INFO("stop car~~~~~,stop_tem %d,stop_car_flag %d,stop_path %d", stop_tem_flag, stop_car_flag, stop_path_flag);
        // ROS_INFO("D10$stop pub $ t_v %d,d_v %d,gear %d,ad %d", quit_cmd.turn_value, quit_cmd.drive_value,quit_cmd.gear_model,quit_cmd.ad_control_enable);
        ...
        if (stop_num > 10 && stop_path_flag) // TODO @cnk
        {
            ...
            if (!planning_result.x.empty())
            {
                ...
                path_too_far_pub_.publish(point_msg);
                // ROS_INFO("Published path_too_far_point: x = %.2f, y = %.2f", point_msg.x, point_msg.y);
            }
            else
            {
                ROS_WARN("Planning result is empty. Cannot publish path_too_far_point.");
            }
        }
    }
    else
    {
        last_turn_value = cmd_msg.turn_value;    //@xhj: 1225 add
        cmd_msg.header.stamp = ros::Time::now(); // 设置时间戳
        pub_Command.publish(cmd_msg);
        // ROS_INFO("D10$cmd pub $ t_v %d,d_v %d,gear %d,ad %d", cmd_msg.turn_value, cmd_msg.drive_value, cmd_msg.gear_model,cmd_msg.ad_control_enable); //@Debug10
    }
}
...
void PurePursuit::controlError_publish()
{
    ...
    // ROS_INFO("CE$ pub $ e_pos %.4f,e_theta_deg %.4f", ctlE_msg.e_pos, ctlE_msg.e_theta);
...
void PurePursuit::initForROS()
{
    ...
    // sub_topics_params
    nh_.param("sub_planning_result_topic", sub_planning_result_topic, std::string("/lawn_mower/global_path")); // 局部规划
    nh_.param("sub_speed_info_topic", sub_speed_info_topic, std::string("/nanobot/localpose"));                // 采用定位的反馈
    nh_.param("sub_pos_topic", sub_pos_topic, std::string("/Mower/position"));                                 // 定位信息
    nh_.param("sub_direct_control_topic", sub_direct_control_topic, std::string("/mower/direct_control"));     // APP控制
    nh_.param("sub_stop_car_topic", sub_stop_car_topic, std::string("/mower/stop_car"));                       // 视觉停障
    nh_.param("sub_avoidstate_topic", sub_avoidstate_topic, std::string("/lawn_mower/avoid_state"));           // 动态避障

    // setup subscriber
    sub1_ = nh_.subscribe(sub_planning_result_topic, 1, &PurePursuit::callbackFromPlanningResult, this);
    sub2_ = nh_.subscribe(sub_speed_info_topic, 1, &PurePursuit::callbackFromSpeedInfo, this);
    sub4_ = nh_.subscribe(sub_pos_topic, 5, &PurePursuit::callbackFromPos, this);
    sub5_ = nh_.subscribe(sub_direct_control_topic, 1, &PurePursuit::callbackFromDirectControl, this);
    substop = nh_.subscribe(sub_stop_car_topic, 1, &PurePursuit::callbackFromTaskstop, this);
    sub_avoid = nh_.subscribe(sub_avoidstate_topic, 1, &PurePursuit::callbackFromAvoidstate, this);
    ...
    path_too_far_pub_ = nh_.advertise<geometry_msgs::Point>("/path_too_far_point", 1); //@cnk
}
...
void PurePursuit::run()
{
    ...
    try
    {
        YAML::Node config = YAML::LoadFile("/home/nvidia/crawler_control/src/pure_pursuit/chassis.yaml"); // 注意路径
        ...
        std::cout << "Basic Config ## m_bool: " << mover_bool_cfg << ", --m_height: " << mower_height_cfg << std::endl;
    ...
    while (ros::ok())
    {
        ...
        /*** 信号触发进入主控制 ***/
        // if (is_planning_result_set_ && is_velocity_set_ && is_carposition_set_)  //驾驶模式临时修改
        ...
        else
        {
            ...
            if (scount > 3 * LOOP_RATE_)
            {
                if (!is_planning_result_set_)
                {
                    ...
                    std::string cmd = "bash -c 'rosnode kill /obstacle_avoidance && sleep 2 && source ~/Mower/devel/setup.bash && roslaunch obstacle_avoidance obstacle_avoidance.launch >> /dev/null 2>&1 &'";
                    int handle = std::system(cmd.c_str());
                    if(handle != 0)
                        ROS_ERROR("Restart fail!");
                    else
                        ROS_WARN("Restart success!");
        ...
        if (tnum > 3 * LOOP_RATE_)
        {
            ...
            // ROS_INFO("[ %s, ]~$$  MAP is  :  %d ,$$ v_info is : %d",timestampToDate(now).c_str(),is_carposition_set_ ,is_velocity_set_ );
            tnum = 0;
        }
...
int calculateDriveValue(float current_speed, float desire_speed, bool flag, uint8_t avoid_state_data)
{
    ...
    // if (avoid_state_data >= 1)
    // {
    //     ROS_INFO("CD_value @ d_v %d,avoid_state.data %d,turn_flag %d", cd_value, avoid_state_data, flag);
    // }
                       ...
                       unsigned char cgoal_id, int last_tv)
{
    ...
    float h_value = latest_gear_flag == 0 ? local_waypose.heading : 0; // 优化转弯点前的 heading 控制 @xhj 25-04-22
    // 0.01 rad ~ 0.573 deg
    if (fabs(c_goal->heading - h) > 0.015 || fabs(c_goal->point.x - local_waypose.point.x) * 100 > 12)
    {
        dev_flag = 1;
        h = c_goal->heading;
    }
    ...
    // ROS_INFO("CT01#dev_flag %d,goal->heading %.3f, CT1# %.3f, CT2# %.2f",dev_flag,c_goal->heading*180/M_PI
    //         ,(c_goal->heading - h)*180/M_PI, (c_goal->point.x - local_waypose.point.x)*100);

    // 优先修正航向偏差 (纠正曲线时)
    if (fabs(local_waypose.point.x) < 0.10 && fabs(-local_waypose.heading) > 0.05 && cu_flag)
    {
        int tmp_value = 0;
        // h_sign ？向左打：向右打
        if (h_sign == 1)
        {
            ...
            tmp_value = last_tv + det_tv < 1420 ? last_tv - det_tv : last_tv + det_tv; // 250312 ：amend
        }
        else
        {
            if (x_sign == 1)
            ...
            tmp_value = last_tv + det_tv > 1580 ? last_tv - det_tv : last_tv + det_tv; // 250312 ：amend
        }
        t_value = tmp_value;
        // ROS_INFO("CT99#last_tv %d,t_value = %d, det_tv = %d", last_tv,t_value, det_tv);
        return t_value;
    }
    ...
    {
        if (fabs(local_waypose.point.x) <= 0.05 && fabs(-local_waypose.heading) <= 0.04)
        {
            t_value = sl_flag ? last_tv : ctv;
        }
        else if (fabs(local_waypose.point.x) > 0.05 || fabs(-local_waypose.heading) > 0.04)
        {
            if (sl_flag)
            {
            ...
            {
                det_tv = static_cast<int>(2 * (100 * local_waypose.point.x - 100 * (h_value))); // 增大横向，减小航向比
    ...
    if (fabs(local_waypose.point.x) >= 0.10 && abs(last_tv - 1500) < 25)
    {
        ...
        t_value = last_tv == 1500 ? ctv : ctv + det_tv;
        // ROS_INFO("CT10#last_tv %d,t_value = %d,det_tv = %d,cgoal_id = %d", last_tv,t_value,det_tv,cgoal_id);
        return t_value;
    }
    // ROS_INFO("CT9#last_tv %d,t_value = %d,det_tv = %d,cgoal_id = %d", last_tv,t_value,det_tv,cgoal_id);
    return t_value;
}
...
std::string getVersion()
{
    ...
    std::string ss;
    /*
        250305起，高速电机——开发log；3月12日，调整；27日 去除标定区;0411 定位话题统一;
        0430 avoid_data = 1处理
    */
