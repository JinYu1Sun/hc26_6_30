    ...
    if (desire_speed_com <= 30.0 && planning_result.point_num > 1) {
      ...
      if (turn_flag) {
        ...
        if (distance_to_start > 0.05 && !adjust_error) {
          if (fabsf(heading_error) > 0.20 && !first_set) {
        ...
        if (fabsf(local_waypoints[0].heading) <= 0.20) // 0.096
        {
        ...
        ROS_WARN("delta_heading,%.4f",
                   planning_result.x[0], planning_result.y[0]);
        if (delta_heading < 0) {
          command.turn_value = -55; // 1270
          command.drive_value = 0; // 1500
          break;
        } else {
          command.turn_value = 55; // 1730
          command.drive_value = 0; // 1500
          break;
        }
      }
  ...
  if (need_turn_for_new_path) {
    if (align_to_path_point && planning_result.pathtype == 0) {
      ...
      if (distance_to_start > 0.05) {
        if (fabsf(heading_error) > 0.20 && !already_moving_straight) {
          command.drive_value = 0; // 1500
          
          command.turn_value = (heading_error > 0) ? - 55 : 55; // 1270 : 1730
    ...
    if (fabsf(heading) <= 0.20) {
      ...
      command.turn_value = (heading > 0) ? -55 : 55; // 1270 : 1730
