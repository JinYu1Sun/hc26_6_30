...
private:
    ...
    // publisher
    // path_too_far_pub_ 历史命名保留，实际发布话题为 /turn_completed，类型 std_msgs::Bool
   ...
   // bool first_path = true;
    ...
    // NONE：    正常跟踪，未遇到转弯点
    // APPROACH：接近 gear=2 转弯点，已减速
    // EXECUTING：到达转弯点，执行原地旋转对准下一段路径
    enum class TurnPhase { NONE, APPROACH, EXECUTING };
    TurnPhase turn_phase_ = TurnPhase::NONE;
    int turn_target_idx_ = -1;  // 当前正在处理的 gear=2 点索引
    ...
    ros::Time turn_completion_guard_end_time_;
