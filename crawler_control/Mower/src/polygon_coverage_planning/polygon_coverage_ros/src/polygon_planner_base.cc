...
void PolygonPlannerBase::advertiseTopics() {
  ...
  clicked_point_sub_ = nh_.subscribe(
      "/clicked_point", 10, &PolygonPlannerBase::clickPointCallback, this);
  polygon_sub_ = nh_.subscribe("/polygon", 1,
                               &PolygonPlannerBase::clickPolygonCallback, this);


  //! ------------- add by py -------------// 
  mark_sub_ = nh_.subscribe("/planning_hull_info",1,
                            &PolygonPlannerBase::markObstacleCallback, this);

  sub_gnss = nh_.subscribe("/send_map_info", 10, &PolygonPlannerBase::GnssCallback,this);
  // 
...
void PolygonPlannerBase::markObstacleCallback(
    const polygon_coverage_msgs::PolygonWithHolesStamped& msg) {

  ROS_INFO("Updating polygon from Andrion polygon tool.");
  PolygonWithHoles temp_pwh;
  double temp_alt;
  if (polygonFromMsg(msg, &temp_pwh, &temp_alt, &global_frame_id_)) {
    ROS_INFO_STREAM("Successfully loaded polygon.");
...
void PolygonPlannerBase::GnssCallback(const geometry_msgs::PointStamped &gpsposition)
{
    
  broadcaster_world.sendTransform(
        tf::StampedTransform(
    ...
    transformStamped->transform.translation.x = gpsposition.point.x;
    transformStamped->transform.translation.y = gpsposition.point.y;
    //transformStamped->transform.translation.x = 1;
    //transformStamped->transform.translation.y = 1;
    transformStamped->transform.translation.z = 0.0;
    tf::Quaternion q;
    q.setRPY(0, 0, gpsposition.point.z);
    //q.setRPY(0, 0, 0);
    ...
    broadcaster_map.sendTransform(*transformStamped);    
