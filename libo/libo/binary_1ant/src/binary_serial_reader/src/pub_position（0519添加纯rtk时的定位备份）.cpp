...
void checkTimeout(const ros::TimerEvent &)
{
    ...
    if ((ros::Time::now() - last_publish_time).toSec() > 5.0)
    {
        ...
        // 重启节点
        int ret = std::system("rosnode kill /serial_reader_node && sleep 2 && rosrun serial_reader read_serial");
        if (ret != 0)
        {
...
int main(int argc, char **argv)
{
    ...
    ros::Timer timer = nh.createTimer(ros::Duration(5.0), checkTimeout);
