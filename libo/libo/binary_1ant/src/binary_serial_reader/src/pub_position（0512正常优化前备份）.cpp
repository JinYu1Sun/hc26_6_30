...
void PubPosition::processSerialData(const std::string &serial_data, ros::Publisher &pub, bool imugood_flag)
{
    ...
    if (confidence == 4 && imugood_flag) // imugood_flag == 1 &&
    {
        ...
        localpose_msg.header.frame_id = "/gps";
        localpose_msg.header.stamp = ros::Time::now();
        localpose_msg.vehicle_speed = (ve > 0 && vn > 0) ? sqrt(ve * ve + vn * vn):0;
    }
    pos_pub.publish(gpsPosition);
    localpose_pub.publish(localpose_msg);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pos_pub = nh.advertise<util::gpsPosition>("/nanobot/gpsposition", 1);
    ros::Publisher localpose_pub = nh.advertise<util::LocalPose>("/nanobot/localpose", 1);
    ...
    while (ros::ok())
    {
        ...
        if (serial.available()) // if (serial.waitReadable())
        {
            ...
            if (serial_data.empty() || serial_data[0] != '#' || ((serial_data.find("DRPVA") == std::string::npos) && (serial_data.find("IMUATTA") == std::string::npos) && (serial_data.find("GNGGA") == std::string::npos)))
            {
            ...
            if (serial_data.find("DRPVA") != std::string::npos && gga_gps_flag == 4) // TODO：&& imugood_flag = true;???根据imu未收敛时的定位好坏来判断
            {
