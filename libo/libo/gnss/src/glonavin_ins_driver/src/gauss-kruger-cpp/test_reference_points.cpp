...
int main(int argc, char* argv[])
{
    ...
    double lon[6] = {114.0770841, 114.0773758, 114.0769008, 114.0762443, 114.0761607, 114.076798 };
    double lat[6] = {30.4474191,  30.4478818,  30.4483617,  30.448,      30.4473187 , 30.4473852};
    for(int i=0; i<6; i++)
    {
        double gauss_y, gauss_x;
        double lat_new, lon_new;
        printf("data[%d]:\n",i);
        printf("Lat=%.8lf, Lon=%.8lf\n", lat[i], lon[i]);
        projection.geodeticToGrid(lat[i], lon[i], gauss_x, gauss_y);
        printf("gauss_x=%.3lf, gauss_y=%.3lf\n", gauss_x, gauss_y);
        projection.gridToGeodetic(gauss_x, gauss_y, lat_new, lon_new);
        printf("LatNew=%.8lf, LonNew=%.8lf\n", lat_new, lon_new);
        printf("dLat=%.8lf, dLon=%.8lf\n", lat_new-lat[i], lon_new-lon[i]);
    }
    return 0;
}
