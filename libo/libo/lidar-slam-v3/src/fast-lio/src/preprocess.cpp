...
// TODO: Shang.此处point_filter_num表示等样抽取数据的间隔.
...
void Preprocess::avia_handler(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
    ...
    pl_filtered.reserve(plsize);

    // 感知分支：line/tag 合法性筛选后，剔除与前一个合法输入点近似重复
    // 的点；不做 point_filter_num 抽点、时间排序或 IMU 去畸变。
    bool has_previous_valid_point = false;
    PointType previous_valid_point;
    for (uint i = 1; i < plsize; i++)
    {
        ...
        point.normal_y = 0.0f;
        point.normal_z = 0.0f;
        point.curvature = msg->points[i].offset_time / float(1000000);

        const bool is_approximately_duplicate =
            has_previous_valid_point &&
            std::abs(point.x - previous_valid_point.x) <= 1e-7 &&
            std::abs(point.y - previous_valid_point.y) <= 1e-7 &&
            std::abs(point.z - previous_valid_point.z) <= 1e-7;

        previous_valid_point = point;
        has_previous_valid_point = true;
        if (!is_approximately_duplicate)
        {
            pl_filtered.push_back(point);
        }
    }
