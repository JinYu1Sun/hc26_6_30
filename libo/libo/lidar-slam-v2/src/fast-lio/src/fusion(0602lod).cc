...
#include <nav_msgs/Path.h> //0602
#include <geometry_msgs/PoseStamped.h> //0602
#include <queue>
#include <ros/ros.h>
...
public:
  FusionNode() : nh_("~") {
    ...
    pubDebugGPSPath_ = nh_.advertise<nav_msgs::Path>("/Mower/debug/gps_path", 1); //0602
    pubDebugLIOPath_ = nh_.advertise<nav_msgs::Path>("/Mower/debug/lio_path", 1); //0602
    gps_path_.header.frame_id = "map"; //0602
    lio_path_.header.frame_id = "map"; //0602
#endif
  }
...
private:
  ...
  void LioCallBack(const util::LIOPoseConstPtr &msg) {
    ...
    if (!reboot && pub_fusion == 3 && lio_state_) {
      ...
      lio_position.position_z = lio_t.z();
    ...
    if (lio_state_ && lio_speed_flag_) {
          ...
          20) // 可能导致当RTK数据较差时只能重启节点但由于无法得到RTK定位而无法得到定位结果，仅重启/lasermapping？？？
      {
        ...
        if (std::system(
                "restart_ros_nodes.sh /fusion /laserMapping -- bash -c 'source "
                "/home/nvidia/libo/lidar-slam-v2/devel/setup.bash && roslaunch "
                "fast_lio location_mode.launch'") != 0) {
    ...
    if (gps_good_flag && system_init_) {
      globalEstimator_.InputGPS(lio_stamp_, latitude, longitude, altitude,
                                gauss_yaw, 0.1);
      inputGPS_flag_ = true;
    }
    ...
    if (inputLIO_flag_) {
      ...
      fusion_pos_[2] =
          inputGPS_flag_ ? location_gps[2] : xyy_lio[2]; // 融合定位的航向
    }
    ...
    if (!reboot && pub_fusion == 1) {
      ...
      // +++++++++++调整前后帧时间间隔的有效判断
      if (0 < time_diff && time_diff < 0.15 && isValidarray(fusion_pos_, 3) &&
          isValidarray(last_fusion_pos_, 3)) {
        fusion_speed =
    ...
    pubDebugGPS_.publish(gps2xyz_msg);
        ...
        // GPS TF + Path（仅在 GPS 数据有效时发布） //0602
    if (gps_good_flag)
    {
        tf::Quaternion gps_q;
        gps_q.setRPY(0, 0, xyy_g[2]);
        tf::Transform gps_tf;
        gps_tf.setOrigin(tf::Vector3(xyy_g[0], xyy_g[1], 0));
        gps_tf.setRotation(gps_q);
        tf_broadcaster_.sendTransform(
            tf::StampedTransform(gps_tf, gps_msg_stamp, "map", "gps_debug"));

        geometry_msgs::PoseStamped gps_pose;
        gps_pose.header.frame_id = "map";
        gps_pose.header.stamp = gps_msg_stamp;
        gps_pose.pose.position.x = xyy_g[0];
        gps_pose.pose.position.y = xyy_g[1];
        gps_pose.pose.position.z = 0;
        tf::quaternionTFToMsg(gps_q, gps_pose.pose.orientation);
        gps_path_.header.stamp = gps_msg_stamp;
        gps_path_.poses.push_back(gps_pose);
        if (gps_path_.poses.size() > 10000)
            gps_path_.poses.erase(gps_path_.poses.begin());
        pubDebugGPSPath_.publish(gps_path_);
    } //0602
    ...
    if (lio_state_) {
      ...
      pubDebugLIO_.publish(LIO_msg);
            ...
            // LIO TF + Path //0602
      tf::Quaternion lio_q;
      lio_q.setRPY(0, 0, xyy_lio[2]);
      tf::Transform lio_tf;
      lio_tf.setOrigin(tf::Vector3(xyy_lio[0], xyy_lio[1], 0));
      lio_tf.setRotation(lio_q);
      tf_broadcaster_.sendTransform(
          tf::StampedTransform(lio_tf, msg->header.stamp, "map", "lio_debug"));

      geometry_msgs::PoseStamped lio_pose;
      lio_pose.header.frame_id = "map";
      lio_pose.header.stamp = msg->header.stamp;
      lio_pose.pose.position.x = xyy_lio[0];
      lio_pose.pose.position.y = xyy_lio[1];
      lio_pose.pose.position.z = 0;
      tf::quaternionTFToMsg(lio_q, lio_pose.pose.orientation);
      lio_path_.header.stamp = msg->header.stamp;
      lio_path_.poses.push_back(lio_pose);
      if (lio_path_.poses.size() > 10000)
          lio_path_.poses.erase(lio_path_.poses.begin());
      pubDebugLIOPath_.publish(lio_path_); //0602
...
private:
    ...
    if (latitude != 0. && longitude != 0. && altitude != 0. && gps_flag == 4) {
      ...
      if (last_gps_stamp_ && gps_stamp) {
        ...
        if ((0.08 < gps_stamp_diff && gps_stamp_diff < 0.22) &&
            isValidarray(location_gps, 3) && isValidarray(last_gps_, 3)) {
  ...
  void GPS2Local(double latitude, double longitude, double height,
    ...
    xyy[0] = xyz[0] * cos(origin_gauss_yaw_ * DEG2RAD) +
             xyz[1] * sin(origin_gauss_yaw_ * DEG2RAD);
    xyy[1] = -xyz[0] * sin(origin_gauss_yaw_ * DEG2RAD) +
             xyz[1] * cos(origin_gauss_yaw_ * DEG2RAD);
  ...
  void LIOPrecess(Eigen::Vector3d &lio_t, Eigen::Quaterniond &lio_q,
    ...
    xyy_lio[0] = lio_t.x() * cos(xy_offset_[2] * DEG2RAD) -
                 (lio_t.y()) * sin(xy_offset_[2] * DEG2RAD) + xy_offset_[0];
    xyy_lio[1] = lio_t.x() * sin(xy_offset_[2] * DEG2RAD) +
                 (lio_t.y()) * cos(xy_offset_[2] * DEG2RAD) + xy_offset_[1];
...
private:
  ...
  ros::Publisher pubDebugGPSPath_, pubDebugLIOPath_; //0602
  tf::TransformBroadcaster tf_broadcaster_; //0602
  nav_msgs::Path gps_path_; //0602
  nav_msgs::Path lio_path_; //0602
#endif
};
