...
private:
    ...
    void LioCallBack(const util::LIOPoseConstPtr &msg)
    {
        ...
        {
            lio_failed_cnt_++;
            if (location_mode_ && lio_failed_cnt_ >= 5)
            {
                reboot = true;
                return;
            }
        ...
        {   printf("lio_stamp = %15.4f",lio_stamp_);
            util::GpsPositionConstPtr gps_msg = gps_queue_.front();
            gps_msg_stamp = gps_msg->header.stamp;
            gps_stamp = gps_msg->header.stamp.toSec();
	    printf("gps_stamp = %15.4f",gps_stamp);
            printf("gps_stamp -lio_stamp = %15.4f",gps_stamp-lio_stamp_);
            if (gps_stamp >= lio_stamp_ - 0.03 && gps_stamp <= lio_stamp_ + 0.03)
            {
            ...
            else if (gps_stamp < lio_stamp_ - 0.03)
                gps_queue_.pop();
            else if (gps_stamp > lio_stamp_ + 0.03)
                break;
        }
        ...
        // gps_good_flag：当前帧或漂移期设置的LIO的状态，不能代表当前帧的GPS状态
        ...
        std::cout << "inputGPS_flag_ = " << inputGPS_flag_ << std::endl;
        ...
        std::cout << "gps_failed_cnt_ = " << gps_failed_cnt_ << std::endl;
        std::cout << "gps_failed_flag_ = " << gps_failed_flag_ << std::endl;
...
private:
    ...
    void SaveFusionPositionToFile(const double *fusion_pos)
    {
        ...
        file.close();
        ROS_INFO("Fusion position saved to file: %s", last_pub_pos_path.c_str());
    }
