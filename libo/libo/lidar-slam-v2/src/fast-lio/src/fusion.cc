...
private:
  ...
  void LioCallBack(const util::LIOPoseConstPtr &msg) {
    ...
    if (inputLIO_flag_) {
      ...
      fusion_pos_[2] =
          inputGPS_flag_ ? location_gps[3] : xyy_lio[3]; // 融合定位的航向
    }
...
private:
  ...
  void GPS2Local(double latitude, double longitude, double height,
    ...
    // xyy[2] 改存相对原点高程(ENU up，即上面 Forward 得到的 xyz[2])，
    // 航向移到 xyy[3]。所有传入本函数的位置数组必须为 4 维。
    xyy[2] = xyz[2];
    xyy[3] = yaw;
  }
  ...
  void LIOPrecess(Eigen::Vector3d &lio_t, Eigen::Quaterniond &lio_q,
    ...
    // [2] 存高程：绕 z 的对齐旋转不改变 z，直接取锚定后的 lio_t.z()；航向移到 [3]。
    xyy_lio[2] = lio_t.z();
    xyy_lio[3] = ConvertYaw(xy_offset_[2] * DEG2RAD + yaw_t);
  }
