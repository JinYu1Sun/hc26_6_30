...
constexpr auto OFFSET = M_PI / 2.0;
int cnt = 0;
...
double calculateAngle(const Vector &v1, const Vector &v2)
{
    double dot = dotProduct(v1, v2);
    double magnitude1 = std::sqrt(v1.x * v1.x + v1.y * v1.y);
    double magnitude2 = std::sqrt(v2.x * v2.x + v2.y * v2.y);
    double cosineTheta = dot / (magnitude1 * magnitude2);
    cosineTheta = std::max(-1.0, std::min(1.0, cosineTheta)); // 限制cos值在[-1, 1]之间

    return std::acos(cosineTheta) * 180.0 / M_PI; // 返回角度值
}

bool areDirectionsConsistent(const Vector &v1, const Vector &v2, double angleThreshold = 45.0)
{
...
private:
    ...
    void LioCallBack(const fast_lio::LIOPoseConstPtr &msg)
    {
        ...
        while (!gps_queue_.empty())
        {
            ...
            if (gps_stamp >= lio_stamp - 0.01 && gps_stamp <= lio_stamp + 0.01)
            {
            ...
            else if (gps_stamp < lio_stamp - 0.01)
                gps_queue_.pop();
            else if (gps_stamp > lio_stamp + 0.01)
                break;
        }
        ...
        if (!system_init_)
        {
            if (gps_flag != 4)
            {
                ...
                if (fail_cnt_ >= fail_threshold_)
                {
                    ...
                    fusion_position.position_x = FLT_MAX;
                    fusion_position.position_y = FLT_MAX;
                    fusion_position.position_z = FLT_MAX;
                    fusion_position.roll = 0;
                    fusion_position.pitch = 0;
                    fusion_position.yaw = 0;
                    fusion_position.position_state = 0;
            ...
            else
            {
                ...
                if (isValue(latitude, longitude, altitude))
        ...
        else
        {
            if (gps_flag == 4)
            {
                ...
                if (restore_flag_)
                {
                    ...
                    // 原始航向角约束
                    double delta_yaw = fabs(xyy_lio[2] - xyy_t[2]);

                    // 平均角度约束
                    double average_angle = 0.;
                    for (int i = 0; i < 5; ++i)
                        average_angle += angle_arry[i];
                    average_angle /= 5;
                    angle_arry[cnt % 5] = calculateAngle(direction1, direction2);

                    // 距离约束
                    double distance = ComputeDistance(xyy_lio, xyy_t);

                    // TODO: 增加一个机制，避免LIO完全失效的时候无法将
                    // GPS定位信息丢入优化器中导致没有定位输出的问题。
                    //  if (switch_flag_)
                    if (switch_flag_)
                    {
                        // 向量夹角、偏航角、平均角度、距离四重判断.
                        if (lio_state_)
                        {
                            if (areDirectionsConsistent(direction1, direction2, 30) && delta_yaw < 0.5 && average_angle < 30.0 && fabs(distance) < 2.0)
                                if (isValue(latitude, longitude, altitude))
                                    globalEstimator.InputGPS(lio_stamp, latitude, longitude, altitude, gauss_yaw, 1);//这三个有何区别？
                        }
                        else
                        {
                            if (isValue(latitude, longitude, altitude))
        ...
        double xyy_p[3];
        if (cnt < 100 && !switch_flag_)
        {
            if (isValue(latitude, longitude, altitude))
                GPS2Local(latitude, longitude, altitude, gauss_yaw, xyy_p);

            if (location_mode_ && !onece_flag_)
            {
                ROS_INFO("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
                offset_yaw_ = xyy_p[2];// -OFFSET;
                onece_flag_ = true;
                std::cout << "<=========== offset_yaw: " << offset_yaw_ << " ===============>" << std::endl;
            }
        }
        else
        {
        ...
        // location mode process.
        if (location_mode_ && switch_flag_)
        {
            xyy_p[2] += offset_yaw_;
            ConvertYaw(xyy_p[2]);
        }
        ...
        fusion_position.header.stamp = msg->header.stamp;
        fusion_position.header.frame_id = "base_link";
        fusion_position.position_x = xyy_p[1];//xyy_p[0];
        fusion_position.position_y = xyy_p[0];//xyy_p[1];
...
private:
    void ParameterInit()
    {
        ...
        memset(angle_arry, 0, sizeof(angle_arry));
    }
