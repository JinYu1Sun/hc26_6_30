...
// 用于优先队列的比较函数：按节点索引访问公共节点池
struct NodeIndexCompare {
    const std::vector<Node>* nodes;

    explicit NodeIndexCompare(const std::vector<Node>* nodes_ptr) : nodes(nodes_ptr) {}
...
private:
    ...
    int map_width_{0};              // 地图宽度(像素)，现在根据边界和分辨率自动计算
    int map_height_{0};             // 地图高度(像素)，现在根据边界和分辨率自动计算
    ...
    std::atomic<bool> reset_requested_{false}; // A* 搜索取消标志，避免 reset() 被正在执行的规划阻塞
    ...
    nav_msgs::Path no_first_avoid_relay_path_; // 连续避障所依赖的寻找安全终点的路径
    std::vector<ObstacleMsg> obstacles_;      // 使用自定义障碍物消息
    std::mutex obstacles_mutex_; // 保护障碍物数据的互斥量
    // ros::Time clear_obstacles_time_;
    std::vector<geometry_msgs::Point> costmap_corners_; // 感知的视野区域
    ...
    std::atomic<bool> backup_success_; // 倒车完成的标志；
    ...
    util::Obstacle inflateObstacle(const util::Obstacle &obstacle, const double &inflation_distance);
    ...
    bool isCollision(int pre_pre_x, int pre_pre_y, int pre_x, int pre_y, int x, int y);
    bool isValidPoint(int x, int y);
    std::vector<Node> reconstructPath(std::size_t goal_index) const;
    ...
    std::pair<int, int> findSafeGoalPoint2(const nav_msgs::Path &path);
    ...
    bool isObstacleInView(const util::Obstacle &obstacle, const std::vector<geometry_msgs::Point> &costmap_corners);
    bool isPositionInBoundary(double x, double y, const std::vector<geometry_msgs::Point> &boundary); //边界检测函数
    ...
    // 判断障碍物碰撞
    bool intersectsWithObstacle(double px, double py, double yaw, double next_yaw, const ObstacleMsg& obstacle);
    double cross(const double& ax, const double& ay, const double& bx, const double& by, const double& px, const double& py);
    bool isPointInQuadrilateral(const double& px, const double& py, const ObstacleMsg &obstacle);
    ...
    std::vector<Node> a_star_nodes_; // A* 搜索中所有节点统一保存在此节点池中，回溯使用索引而不是裸指针
    ...
    static constexpr double STATE4_TIMEOUT_SECONDS = 5.0; // 状态4超时时间（秒）
};
